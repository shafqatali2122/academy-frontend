...
const Header = () => {
  ...
  // --- SAFE ADMIN CHECK (ONLY LOGIC CHANGE) ---
  const adminRoles = ['SuperAdmin', 'AdmissionsAdmin', 'ContentAdmin', 'AudienceAdmin'];
        ...
        <div className="hidden md:flex items-center space-x-6">
          ...
          {user ? (
            ...
            <div className="flex items-center space-x-4 border-l pl-4 border-gray-300">
              <Link
                href={isAdmin ? '/admin/dashboard' : '/my-dashboard'}
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`block w-full text-gray-700 hover:text-blue-700 ${
                  router.pathname === item.path ? 'font-semibold text-blue-700' : 'font-medium'
                }`}
              >
                {item.name}
              </Link>
              ...
              {user ? (
                <>
                  <Link
                    href={isAdmin ? '/admin/dashboard' : '/my-dashboard'}
                    onClick={() => setIsOpen(false)}
                    className="block w-full font-medium text-gray-700 hover:text-blue-700"
                  >
                    My Dashboard
                  </Link>
                  <button
